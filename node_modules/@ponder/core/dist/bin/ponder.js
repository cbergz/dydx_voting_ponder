...
// src/bin/ponder.ts
var ponder_exports = {};
module.exports = __toCommonJS(ponder_exports);
...
var buildOptions = ({
  ...
  const defaults = {
    rootDir: import_node_path.default.resolve(cliOptions.rootDir),
  ...
  return {
    ...defaults,
    // Resolve paths
    configFile: import_node_path.default.join(defaults.rootDir, defaults.configFile),
    schemaFile: import_node_path.default.join(defaults.rootDir, defaults.schemaFile),
    srcDir: import_node_path.default.join(defaults.rootDir, defaults.srcDir),
    generatedDir: import_node_path.default.join(defaults.rootDir, defaults.generatedDir),
    ponderDir: import_node_path.default.join(defaults.rootDir, defaults.ponderDir)
  };
};

// src/config/ponderConfig.ts
...
var ensureDirExists = (filePath) => {
  ...
  (0, import_node_fs.mkdirSync)(dirname, { recursive: true });
};

// src/config/ponderConfig.ts
var buildPonderConfig = async ({
  configFile
}) => {
  if (!(0, import_node_fs2.existsSync)(configFile)) {
    throw new Error(`Ponder config file not found, expected: ${configFile}`);
  }
  const buildFile = import_path.default.join(import_path.default.dirname(configFile), "__ponder__.js");
  ensureDirExists(buildFile);
  (0, import_node_fs2.rmSync)(buildFile, { force: true });
  try {
    ...
    if (!rawConfig) {
      if (rawDefault) {
        throw new Error(
          `Ponder config not found. ${import_path.default.basename(
            configFile
      ...
      switch (field.kind) {
        ...
        case 2 /* LIST */: {
          ...
          if (Object.keys(gqlScalarToTsType).includes(
            ...
            // @ts-ignore
            field.baseGqlType.astNode?.kind === import_graphql.Kind.ENUM_TYPE_DEFINITION
          ) {
            const enumValues = (
              // @ts-ignore
...
// src/codegen/service.ts
var CodegenService = class extends import_emittery.default {
  resources;
  contracts;
  logFilters;
  constructor({
  ...
  generateAppFile({ schema } = {}) {
    ...
    const filePath = import_node_path3.default.join(this.resources.options.generatedDir, "index.ts");
    ensureDirExists(filePath);
    (0, import_node_fs3.writeFileSync)(filePath, final, "utf8");
  }
  generateSchemaFile({ graphqlSchema }) {
    const header = `
      """ Autogenerated file. Do not edit manually. """
    `;
    const body = (0, import_graphql2.printSchema)(graphqlSchema);
...
var buildSingleAbi = ({
  ...
  if (typeof abiConfig === "string") {
    filePath = import_node_path4.default.isAbsolute(abiConfig) ? abiConfig : import_node_path4.default.join(import_node_path4.default.dirname(configFilePath), abiConfig);
    const abiString = (0, import_node_fs4.readFileSync)(filePath, "utf-8");
...
var buildDatabase = ({
  ...
  const defaultSqliteFilename = import_node_path5.default.join(options.ponderDir, "cache.db");
  if (config.database) {
...
var ErrorService = class extends import_emittery2.default {
...
var EventAggregatorService = class extends import_emittery3.default {
  eventStore;
  logFilters;
  networks;
...
// src/historical-sync/service.ts
var HistoricalSyncService = class extends import_emittery4.default {
  metrics;
  eventStore;
  logFilters;
  network;
...
// src/realtime-sync/service.ts
var RealtimeSyncService = class extends import_emittery5.default {
  metrics;
  eventStore;
  logFilters;
  network;
  ...
  blockTaskWorker = async (block) => {
    ...
    if (newBlock.number > previousHeadBlock.number + 1) {
      ...
      for (const block2 of [...missingBlocks, newBlockWithTransactions].sort(
        (a, b) => (0, import_viem8.hexToNumber)(a.number) - (0, import_viem8.hexToNumber)(b.number)
      )) {
...
var gqlScalarTypeByName = {
  ID: import_graphql3.GraphQLID,
  Int: import_graphql3.GraphQLInt,
  Float: import_graphql3.GraphQLFloat,
  String: import_graphql3.GraphQLString,
  Boolean: import_graphql3.GraphQLBoolean,
  BigInt: import_graphql3.GraphQLString,
  Bytes: import_graphql3.GraphQLString
};
...
var unwrapFieldDefinition = (field) => {
  ...
  if (fieldType.kind === import_graphql3.Kind.NON_NULL_TYPE) {
    isNotNull = true;
    fieldType = fieldType.type;
  }
  if (fieldType.kind === import_graphql3.Kind.LIST_TYPE) {
    isList = true;
    fieldType = fieldType.type;
    if (fieldType.kind === import_graphql3.Kind.NON_NULL_TYPE) {
...
var getEntityTypes = (schema) => {
  const entities = Object.values(schema.getTypeMap()).filter((type) => {
    return type.astNode?.kind === import_graphql3.Kind.OBJECT_TYPE_DEFINITION;
  }).filter((type) => {
...
var getUserDefinedScalarTypes = (schema) => {
  return Object.values(schema.getTypeMap()).filter(
    (type) => !!type.astNode && type.astNode.kind === import_graphql3.Kind.SCALAR_TYPE_DEFINITION && !["BigInt", "Bytes"].includes(type.name)
  );
};
var getEnumTypes = (schema) => {
  return Object.values(schema.getTypeMap()).filter(
    (type) => !!type.astNode && type.astNode.kind === import_graphql3.Kind.ENUM_TYPE_DEFINITION
  );
};
        ...
        switch (field.kind) {
          case 0 /* SCALAR */: {
            fieldConfigMap[field.name] = {
              type: field.notNull ? new import_graphql4.GraphQLNonNull(field.scalarGqlType) : field.scalarGqlType,
          ...
          case 1 /* ENUM */: {
            fieldConfigMap[field.name] = {
              type: field.notNull ? new import_graphql4.GraphQLNonNull(field.enumGqlType) : field.enumGqlType
            };
            break;
          }
          case 3 /* RELATIONSHIP */: {
          ...
          case 4 /* DERIVED */: {
            ...
            fieldConfigMap[field.name] = {
              type: new import_graphql4.GraphQLNonNull(
                new import_graphql4.GraphQLList(
                  new import_graphql4.GraphQLNonNull(entityTypes[field.baseGqlType.name])
                )
              ),
              resolve: resolver
            };
          ...
          case 2 /* LIST */: {
            const listType = new import_graphql4.GraphQLList(
              field.isListElementNotNull ? new import_graphql4.GraphQLNonNull(field.baseGqlType) : field.baseGqlType
            );
            fieldConfigMap[field.name] = {
              type: field.notNull ? new import_graphql4.GraphQLNonNull(listType) : listType
            };
            break;
          }
        }
...
var buildPluralField = (entity, entityType) => {
    ...
    switch (field.kind) {
      case 0 /* SCALAR */: {
        ...
        operators.singular.forEach((suffix) => {
          filterFields[`${field.name}${suffix}`] = {
            type: new import_graphql5.GraphQLList(field.scalarGqlType)
          };
        });
      ...
      case 1 /* ENUM */: {
        ...
        operators.singular.forEach((suffix) => {
          filterFields[`${field.name}${suffix}`] = {
            type: new import_graphql5.GraphQLList(field.enumGqlType)
          };
        });
        break;
      }
      case 2 /* LIST */: {
        operators.universal.forEach((suffix) => {
          filterFields[`${field.name}${suffix}`] = {
            type: new import_graphql5.GraphQLList(field.baseGqlType)
          };
        });
      ...
      case 3 /* RELATIONSHIP */: {
        operators.universal.forEach((suffix) => {
          filterFields[`${field.name}${suffix}`] = { type: import_graphql5.GraphQLID };
        });
        operators.numeric.forEach((suffix) => {
          filterFields[`${field.name}${suffix}`] = {
            type: import_graphql5.GraphQLID
          };
        });
        operators.singular.forEach((suffix) => {
          filterFields[`${field.name}${suffix}`] = {
            type: new import_graphql5.GraphQLList(import_graphql5.GraphQLID)
          };
        });
        operators.string.forEach((suffix) => {
          filterFields[`${field.name}${suffix}`] = {
  ...
  const filterType = new import_graphql5.GraphQLInputObjectType({
  ...
  return {
    type: new import_graphql5.GraphQLNonNull(new import_graphql5.GraphQLList(new import_graphql5.GraphQLNonNull(entityType))),
    args: {
      where: { type: filterType },
      first: { type: import_graphql5.GraphQLInt },
      skip: { type: import_graphql5.GraphQLInt },
      orderBy: { type: import_graphql5.GraphQLString },
      orderDirection: { type: import_graphql5.GraphQLString }
    },
    resolve: resolver
  };
};
...
var buildSingularField = (entity, entityType) => {
  ...
  return {
    type: entityType,
    args: {
      id: { type: new import_graphql6.GraphQLNonNull(import_graphql6.GraphQLID) }
    },
    resolve: resolver
  };
};
...
var buildGqlSchema = (schema) => {
  ...
  const queryType = new import_graphql7.GraphQLObjectType({
    name: "Query",
    fields: queryFields
  });
  const gqlSchema = new import_graphql7.GraphQLSchema({
    query: queryType
  });
  return gqlSchema;
};
...
var readGraphqlSchema = ({ options }) => {
  const schemaBody = (0, import_node_fs5.readFileSync)(options.schemaFile);
  const schemaSource = schemaHeader + schemaBody.toString();
  const schema = (0, import_graphql8.buildSchema)(schemaSource);
  return schema;
};
...
var readHandlers = async ({
  ...
  const entryAppFilename = import_node_path6.default.join(options.generatedDir, "index.ts");
  if (!(0, import_node_fs6.existsSync)(entryAppFilename)) {
    throw new Error(
  ...
  const buildDir = import_node_path6.default.join(options.ponderDir, "out");
  (0, import_node_fs6.rmSync)(buildDir, { recursive: true, force: true });
  try {
    ...
    const error = err;
    const stackTraces = (0, import_esbuild2.formatMessagesSync)(error.errors, {
      kind: "error",
      color: true
    });
  ...
  const tsconfigPath = import_node_path6.default.join(options.rootDir, "tsconfig.json");
  ...
  const outAppFilename = import_node_path6.default.join(buildDir, "generated/index.js");
...
// src/reload/service.ts
var ReloadService = class extends import_emittery6.default {
  resources;
  latestFileHashes = {};
  kill;
    ...
    watcher.on("change", async (filePath) => {
      if (filePath === this.resources.options.configFile) {
        this.resources.logger.logMessage(
          "error",
          "detected change in ponder.config.ts. " + import_picocolors.default.bold("Restart the server.")
        );
        this.emit("ponderConfigChanged");
        return;
      }
      if (this.isFileChanged(filePath)) {
        const fileName = import_node_path7.default.basename(filePath);
        this.resources.logger.logMessage(
          "event",
          "detected change in " + import_picocolors.default.bold(fileName)
        );
...
var ServerService = class extends import_emittery7.default {
  resources;
  userStore;
  app;
  terminate;
  ...
  async start() {
    ...
    const resolvedPort = await (0, import_detect_port.default)(this.resources.options.port);
  ...
  reload({ graphqlSchema }) {
    this.graphqlMiddleware = (0, import_express_graphql.graphqlHTTP)({
...
var HandlersBar = ({ ui }) => {
  ...
  const titleText = () => {
    if (isUpToDate)
      return /* @__PURE__ */ import_react2.default.createElement(import_ink2.Text, { color: "greenBright" }, "(up to date)");
    if (isStarted)
      return /* @__PURE__ */ import_react2.default.createElement(import_ink2.Text, { color: "yellowBright" }, "(up to ", ui.handlersToTimestamp === 0 ? "" : dateText, ")");
  ...
  return /* @__PURE__ */ import_react2.default.createElement(import_ink2.Box, { flexDirection: "column" }, /* @__PURE__ */ import_react2.default.createElement(import_ink2.Box, { flexDirection: "row" }, /* @__PURE__ */ import_react2.default.createElement(import_ink2.Text, { bold: true }, "Handlers "), /* @__PURE__ */ import_react2.default.createElement(import_ink2.Text, null, titleText())), /* @__PURE__ */ import_react2.default.createElement(import_ink2.Box, { flexDirection: "row" }, /* @__PURE__ */ import_react2.default.createElement(
    ProgressBar,
    {
...
var HistoricalBar = ({
  ...
  const completionText = Number.isInteger(completionDecimal) && completionDecimal < 100 ? `${completionDecimal}.0%` : `${completionDecimal}%`;
  return /* @__PURE__ */ import_react3.default.createElement(import_ink3.Box, { flexDirection: "column" }, /* @__PURE__ */ import_react3.default.createElement(import_ink3.Text, null, title), /* @__PURE__ */ import_react3.default.createElement(import_ink3.Box, { flexDirection: "row" }, /* @__PURE__ */ import_react3.default.createElement(
    ProgressBar,
    {
      current: cachedBlocks + completedBlocks,
      end: totalBlocks || 1
    }
  ), /* @__PURE__ */ import_react3.default.createElement(import_ink3.Text, null, " ", completionText, etaText)));
};
...
var import_emittery8 = __toESM(require("emittery"));
...
function getInjectedContract({
  ...
  const injectedContract = (0, import_viem9.getContract)({ abi, address, publicClient });
...
function getSourceMappedStackFrame(file, lineNumber, columnNumber) {
  let fileContents;
  try {
    fileContents = (0, import_node_fs8.readFileSync)(file, { encoding: "utf-8" });
  } catch (_) {
    return null;
  }
...
// src/user-handlers/service.ts
var EventHandlerService = class extends import_emittery8.default {
  resources;
  userStore;
...
var LoggerService = class {
  ...
  logMessage(kind, message) {
    ...
    switch (kind) {
      case "event": {
        this.info(import_picocolors2.default.magenta(padded) + " - " + message);
        break;
      }
      case "error": {
        this.error(import_picocolors2.default.red(padded) + " - " + message);
        break;
      }
      case "warning": {
        this.error(import_picocolors2.default.yellow(padded) + " - " + message);
        break;
      }
      case "historical": {
        this.info(import_picocolors2.default.yellow(padded) + " - " + message);
        break;
      }
      case "realtime": {
        this.info(import_picocolors2.default.cyan(padded) + " - " + message);
        break;
      }
      case "indexer": {
        this.info(import_picocolors2.default.blue(padded) + " - " + message);
        break;
      }
    }
  }
    ...
    this.networkSyncServices.forEach((networkSyncService) => {
      ...
      historicalSyncService.on("syncStarted", () => {
        logFilters.forEach(({ name }) => {
          ...
          this.resources.logger.logMessage(
            "historical",
            `started historical sync for ${import_picocolors3.default.bold(name)} (${formatPercentage(
...
var cli = (0, import_cac.cac)("ponder").version(package_default.version).usage("<command> [options]").help().option("--config-file [path]", `Path to config file`, {
...
cli.command("dev", "Start the development server").action(async (cliOptions) => {
  if (cliOptions.help)
    process.exit(0);
  const configFile = import_node_path9.default.resolve(cliOptions.configFile);
