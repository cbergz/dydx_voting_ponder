...
var CodegenService = class extends import_emittery.default {
  ...
  generateAppFile({ schema } = {}) {
    ...
    const filePath = import_node_path2.default.join(this.resources.options.generatedDir, "index.ts");
    ensureDirExists(filePath);
    (0, import_node_fs2.writeFileSync)(filePath, final, "utf8");
  }
...
var buildSingleAbi = ({
  ...
  if (typeof abiConfig === "string") {
    filePath = import_node_path3.default.isAbsolute(abiConfig) ? abiConfig : import_node_path3.default.join(import_node_path3.default.dirname(configFilePath), abiConfig);
    const abiString = (0, import_node_fs3.readFileSync)(filePath, "utf-8");
...
var buildDatabase = ({
  ...
  const defaultSqliteFilename = import_node_path4.default.join(options.ponderDir, "cache.db");
  if (config.database) {
...
var readGraphqlSchema = ({ options }) => {
  const schemaBody = (0, import_node_fs4.readFileSync)(options.schemaFile);
...
var readHandlers = async ({
  ...
  const entryAppFilename = import_node_path5.default.join(options.generatedDir, "index.ts");
  if (!(0, import_node_fs5.existsSync)(entryAppFilename)) {
    throw new Error(
  ...
  const buildDir = import_node_path5.default.join(options.ponderDir, "out");
  (0, import_node_fs5.rmSync)(buildDir, { recursive: true, force: true });
  try {
  ...
  const tsconfigPath = import_node_path5.default.join(options.rootDir, "tsconfig.json");
  ...
  const outAppFilename = import_node_path5.default.join(buildDir, "generated/index.js");
...
var getStackTraceAndCodeFrame = (error, options) => {
  ...
  const buildDir = import_node_path7.default.join(options.ponderDir, "out");
...
function getSourceMappedStackFrame(file, lineNumber, columnNumber) {
  ...
  if (!sourceFileRelative || !sourceLineNumber)
    return null;
  const sourceFile = import_node_path7.default.resolve(import_node_path7.default.dirname(file), sourceFileRelative);
  return {
    sourceFile,
    sourceLineNumber,
    sourceColumnNumber,
