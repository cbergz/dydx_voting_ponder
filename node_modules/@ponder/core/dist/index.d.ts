import { AbiEvent, Address, Abi, ExtractAbiFunctionNames } from 'abitype';
import Emittery from 'emittery';
import { GraphQLObjectType, TypeNode, GraphQLScalarType, GraphQLEnumType, GraphQLInputType, GraphQLInputObjectType, GraphQLSchema } from 'graphql';
import { PublicClient, Hex, Hash, Address as Address$1, AccessList, RpcLog, RpcBlock, RpcTransaction, GetContractReturnType } from 'viem';
import { Kysely } from 'kysely';
import prometheus from 'prom-client';
import express from 'express';

type ResolvedPonderConfig = {
...
type PonderConfig = ResolvedPonderConfig | Promise<ResolvedPonderConfig> | (() => ResolvedPonderConfig) | (() => Promise<ResolvedPonderConfig>);
...
declare class ErrorService extends Emittery<ErrorServiceEvents> {
...
declare class HistoricalSyncService extends Emittery<HistoricalSyncEvents> {
    ...
    constructor({ metrics, eventStore, logFilters, network, }: {
...
declare class LoggerService {
    ...
    private maxWidth;
    logMessage(kind: "event" | "error" | "warning" | "historical" | "realtime" | "indexer", message: string): void;
}
...
declare class ReloadService extends Emittery<ReloadServiceEvents> {
    resources: Resources;
...
type ServerServiceEvents = {
    serverStarted: {
        desiredPort: number;
        port: number;
    };
};
declare class ServerService extends Emittery<ServerServiceEvents> {
    resources: Resources;
    userStore: UserStore;
    app?: express.Express;
    private terminate?;
...
declare class EventHandlerService extends Emittery<EventHandlerEvents> {
    ...
    private schema?;
    private queue?;
    private injectedContracts;
    private eventProcessingMutex;
    private eventsHandledToTimestamp;
...
type ReadOnlyContract<TAbi extends Abi = Abi, _ReadFunctionNames extends string = TAbi extends Abi ? Abi extends TAbi ? string : ExtractAbiFunctionNames<TAbi, "pure" | "view"> : string> = GetContractReturnType<TAbi, PublicClient, unknown, Address, never, _ReadFunctionNames, never>;
...
export { Block, Log, Model, Ponder, PonderApp, PonderConfig, PonderOptions, ReadOnlyContract, ResolvedPonderConfig, Transaction };
