...
var readHandlers = async ({
  ...
  outFilenames.forEach((file) => delete __require.cache[__require.resolve(file)]);
  ...
  const result = __require(outAppFilename);
...
var CodegenService = class extends Emittery {
  ...
  generateAppFile({ schema } = {}) {
    ...
    const filePath = path3.join(this.resources.options.generatedDir, "index.ts");
...
var buildSingleAbi = ({
  ...
  if (typeof abiConfig === "string") {
    filePath = path4.isAbsolute(abiConfig) ? abiConfig : path4.join(path4.dirname(configFilePath), abiConfig);
...
var PostgresError = class extends BaseError {
  ...
  constructor({
    ...
    const params = parameters.reduce(
      (acc, parameter, idx) => {
        acc[idx + 1] = typeof parameter === "string" && parameter.length > 80 ? parameter.slice(0, 80).concat("...") : parameter;
        return acc;
      },
      {}
    );
...
var originalClientQuery = Client.prototype.query;
Client.prototype.query = async function query(...args) {
  try {
...
var buildDatabase = ({
  ...
  const defaultSqliteFilename = path5.join(options.ponderDir, "cache.db");
  if (config.database) {
...
function buildLogFilters({
  ...
  const filterLogFilters = (config.filters ?? []).map((filter) => {
    ...
    const topics = filter.filter.event ? encodeEventTopics({
...
// src/errors/ErrorService.ts
import Emittery2 from "emittery";
var ErrorService = class extends Emittery2 {
  isHandlerError = false;
  submitHandlerError({ error }) {
...
import Emittery3 from "emittery";
import { decodeEventLog } from "viem";
var EventAggregatorService = class extends Emittery3 {
  eventStore;
  logFilters;
  networks;
...
var PostgresEventStore = class {
  ...
  getLogEvents = async ({
    ...
    const handledLogQuery = this.db.with(
      ...
      () => sql2`( values ${sql2.join(filters.map(buildLogFilterValues))} )`
      ...
      ({ and, or, cmpr, ref }) => and([
        ...
        or([
          ...
          cmpr("logFilter_address", "like", sql2`'%' || logs.address || '%'`)
        ]),
        and([
          or([
            cmpr("logFilter_topic0", "is", null),
            cmpr("logFilter_topic0", "like", sql2`'%' || logs.topic0 || '%'`)
          ]),
          or([
            cmpr("logFilter_topic1", "is", null),
            cmpr("logFilter_topic1", "like", sql2`'%' || logs.topic1 || '%'`)
          ]),
          or([
            cmpr("logFilter_topic2", "is", null),
            cmpr("logFilter_topic2", "like", sql2`'%' || logs.topic2 || '%'`)
          ]),
          or([
            cmpr("logFilter_topic3", "is", null),
            cmpr("logFilter_topic3", "like", sql2`'%' || logs.topic3 || '%'`)
          ])
        ]),
        or([
...
function buildLogFilterValues(filter) {
  ...
  return sql2`(${sql2.join([
    sql2.val(name),
    sql2`${sql2.val(chainId)}::integer`,
    sql2.val(address_),
    sql2.val(topic0),
    sql2.val(topic1),
    sql2.val(topic2),
    sql2.val(topic3),
    sql2`${sql2.val(fromBlock ? intToBlob(fromBlock) : null)}::bytea`,
    sql2`${sql2.val(toBlock ? intToBlob(toBlock) : null)}::bytea`,
    sql2.val(handledTopic0_)
  ])})`;
}

// src/event-store/sqlite/store.ts
import { Kysely as Kysely3, Migrator as Migrator2, NO_MIGRATIONS as NO_MIGRATIONS2, sql as sql3, SqliteDialect } from "kysely";
...
function rpcToSqliteTransaction(transaction) {
  return {
    ...
    maxFeePerGas: transaction.maxFeePerGas ? intToBlob(transaction.maxFeePerGas) : null,
    maxPriorityFeePerGas: transaction.maxPriorityFeePerGas ? intToBlob(transaction.maxPriorityFeePerGas) : null,
    nonce: hexToNumber2(transaction.nonce),
    r: transaction.r,
    s: transaction.s,
    to: transaction.to ? transaction.to : null,
    transactionIndex: Number(transaction.transactionIndex),
...
var SqliteEventStore = class {
  ...
  migrateDown = async () => {
    const { error } = await this.migrator.migrateTo(NO_MIGRATIONS2);
    if (error)
      throw error;
  };
  insertUnfinalizedBlock = async ({
  ...
  getLogEvents = async ({
    ...
    const handledLogQuery = this.db.with(
      "logFilters(logFilter_name, logFilter_chainId, logFilter_address, logFilter_topic0, logFilter_topic1, logFilter_topic2, logFilter_topic3, logFilter_fromBlock, logFilter_toBlock, logFilter_handledTopic0)",
      () => sql3`( values ${sql3.join(filters.map(buildLogFilterValues2))} )`
    ).selectFrom("logs").leftJoin("blocks", "blocks.hash", "logs.blockHash").leftJoin("transactions", "transactions.hash", "logs.transactionHash").innerJoin("logFilters", (join) => join.onTrue()).select([
      "logFilter_name",
      "logs.address as log_address",
      "logs.blockHash as log_blockHash",
      ...
      ({ and, or, cmpr, ref }) => and([
        ...
        or([
          cmpr("logFilter_address", "is", null),
          cmpr("logFilter_address", "like", sql3`'%' || logs.address || '%'`)
        ]),
        and([
          or([
            cmpr("logFilter_topic0", "is", null),
            cmpr("logFilter_topic0", "like", sql3`'%' || logs.topic0 || '%'`)
          ]),
          or([
            cmpr("logFilter_topic1", "is", null),
            cmpr("logFilter_topic1", "like", sql3`'%' || logs.topic1 || '%'`)
          ]),
          or([
            cmpr("logFilter_topic2", "is", null),
            cmpr("logFilter_topic2", "like", sql3`'%' || logs.topic2 || '%'`)
          ]),
          or([
            cmpr("logFilter_topic3", "is", null),
            cmpr("logFilter_topic3", "like", sql3`'%' || logs.topic3 || '%'`)
          ])
        ]),
        or([
          cmpr("logFilter_fromBlock", "is", null),
...
function buildLogFilterValues2(filter) {
  ...
  return sql3`(${sql3.join([
    sql3.val(name),
    sql3`cast (${sql3.val(chainId)} as integer)`,
    sql3.val(address_),
    sql3.val(topic0),
    sql3.val(topic1),
    sql3.val(topic2),
    sql3.val(topic3),
    sql3`cast (${sql3.val(fromBlock ? intToBlob(fromBlock) : null)} as blob)`,
    sql3`cast (${sql3.val(toBlock ? intToBlob(toBlock) : null)} as blob)`,
    sql3.val(handledTopic0_)
  ])})`;
}

// src/historical-sync/service.ts
import Emittery4 from "emittery";
import {
  HttpRequestError,
...
function createQueue({
  ...
  const retryTimeouts = retry.timeouts(
  ...
  queue.addTask = async (task, taskOptions) => {
    ...
    await queue.add(
      async () => {
        let result;
        if (retryTimeout)
          await setTimeout2(retryTimeout, false, { signal });
        try {
...
// src/historical-sync/service.ts
var HistoricalSyncService = class extends Emittery4 {
  metrics;
  eventStore;
  logFilters;
  network;
    ...
    await Promise.all(
      this.logFilters.map(async (logFilter) => {
        ...
        const cachedBlockCount = cachedRanges.reduce(
          (acc, cur) => acc + (Number(cur.endBlock) + 1 - Number(cur.startBlock)),
          0
        );
  ...
  buildQueue = () => {
    ...
    const queue = createQueue({
      ...
      onError: ({ error, task, queue: queue2 }) => {
        ...
        if (task.kind === "LOG_SYNC" && error instanceof InvalidParamsRpcError && error.details.startsWith("Log response size exceeded.")) {
        ...
        if (task.kind === "LOG_SYNC" && error instanceof HttpRequestError && error.details.includes(
  ...
  blockTaskWorker = async ({ task }) => {
    ...
    const { startingRangeEndTimestamp } = await this.eventStore.insertFinalizedBlock({
      chainId: this.network.chainId,
      block,
      transactions,
      logFilterRange: {
...
import Emittery5 from "emittery";
import pLimit from "p-limit";
import { hexToNumber as hexToNumber4, numberToHex } from "viem";
...
function rpcBlockToLightBlock(block) {
  return {
    hash: block.hash,
    parentHash: block.parentHash,
    number: hexToNumber3(block.number),
    timestamp: hexToNumber3(block.timestamp)
  };
}

// src/realtime-sync/service.ts
var RealtimeSyncService = class extends Emittery5 {
  metrics;
  eventStore;
  logFilters;
  network;
  ...
  setup = async () => {
    ...
    const finalizedBlockNumber = Math.max(
      0,
      hexToNumber4(latestBlock.number) - this.network.finalityBlockCount
    );
  ...
  blockTaskWorker = async (block) => {
    ...
    if (newBlock.number == previousHeadBlock.number + 1 && newBlock.parentHash == previousHeadBlock.hash) {
      ...
      this.emit("realtimeCheckpoint", {
        timestamp: hexToNumber4(newBlockWithTransactions.timestamp)
      });
    ...
    if (newBlock.number > previousHeadBlock.number + 1) {
      ...
      for (const block2 of [...missingBlocks, newBlockWithTransactions].sort(
        (a, b) => hexToNumber4(a.number) - hexToNumber4(b.number)
      )) {
        this.queue.addTask(block2);
      }
      return;
    }
...
// src/reload/service.ts
import chokidar from "chokidar";
import Emittery6 from "emittery";
    ...
    const fields = gqlFields.map((field) => {
      ...
      if (entityBaseType) {
        ...
        return {
          ...
          relatedEntityIdTypeName: relatedEntityIdType.name
        };
      }
      if (isList) {
        if (scalarBaseType) {
...
var unwrapFieldDefinition = (field) => {
  ...
  if (fieldType.kind === Kind2.NON_NULL_TYPE) {
    isNotNull = true;
    fieldType = fieldType.type;
  }
  if (fieldType.kind === Kind2.LIST_TYPE) {
    isList = true;
    fieldType = fieldType.type;
    if (fieldType.kind === Kind2.NON_NULL_TYPE) {
...
var getEntityTypes = (schema) => {
  const entities = Object.values(schema.getTypeMap()).filter((type) => {
    return type.astNode?.kind === Kind2.OBJECT_TYPE_DEFINITION;
  }).filter((type) => {
...
var getUserDefinedScalarTypes = (schema) => {
  return Object.values(schema.getTypeMap()).filter(
    (type) => !!type.astNode && type.astNode.kind === Kind2.SCALAR_TYPE_DEFINITION && !["BigInt", "Bytes"].includes(type.name)
  );
};
var getEnumTypes = (schema) => {
  return Object.values(schema.getTypeMap()).filter(
    (type) => !!type.astNode && type.astNode.kind === Kind2.ENUM_TYPE_DEFINITION
  );
};

// src/server/graphql/buildGqlSchema.ts
import { GraphQLObjectType as GraphQLObjectType5, GraphQLSchema as GraphQLSchema3 } from "graphql";

// src/server/graphql/buildEntityType.ts
import {
  GraphQLList,
  GraphQLNonNull,
  GraphQLObjectType as GraphQLObjectType2
} from "graphql";
var buildEntityType = (entity, entityTypes) => {
        ...
        switch (field.kind) {
          case 0 /* SCALAR */: {
            fieldConfigMap[field.name] = {
              type: field.notNull ? new GraphQLNonNull(field.scalarGqlType) : field.scalarGqlType,
          ...
          case 1 /* ENUM */: {
            fieldConfigMap[field.name] = {
              type: field.notNull ? new GraphQLNonNull(field.enumGqlType) : field.enumGqlType
            };
            break;
          }
          case 3 /* RELATIONSHIP */: {
          ...
          case 2 /* LIST */: {
            const listType = new GraphQLList(
              field.isListElementNotNull ? new GraphQLNonNull(field.baseGqlType) : field.baseGqlType
            );
            fieldConfigMap[field.name] = {
...
// src/server/graphql/buildPluralField.ts
import {
  GraphQLID as GraphQLID2,
...
var buildPluralField = (entity, entityType) => {
  const filterFields = {};
  entity.fields.forEach((field) => {
    switch (field.kind) {
      case 0 /* SCALAR */: {
        ...
        operators.singular.forEach((suffix) => {
          filterFields[`${field.name}${suffix}`] = {
            type: new GraphQLList2(field.scalarGqlType)
          };
        });
      ...
      case 1 /* ENUM */: {
        ...
        operators.singular.forEach((suffix) => {
          filterFields[`${field.name}${suffix}`] = {
            type: new GraphQLList2(field.enumGqlType)
          };
        });
        break;
      }
      case 2 /* LIST */: {
        operators.universal.forEach((suffix) => {
          filterFields[`${field.name}${suffix}`] = {
            type: new GraphQLList2(field.baseGqlType)
          };
        });
      ...
      case 3 /* RELATIONSHIP */: {
        operators.universal.forEach((suffix) => {
          filterFields[`${field.name}${suffix}`] = { type: GraphQLID2 };
        });
        operators.numeric.forEach((suffix) => {
          filterFields[`${field.name}${suffix}`] = {
            type: GraphQLID2
        ...
        operators.singular.forEach((suffix) => {
          filterFields[`${field.name}${suffix}`] = {
            type: new GraphQLList2(GraphQLID2)
          };
        });
        operators.string.forEach((suffix) => {
          filterFields[`${field.name}${suffix}`] = {
  ...
  const filterType = new GraphQLInputObjectType2({
  ...
  return {
    type: new GraphQLNonNull2(new GraphQLList2(new GraphQLNonNull2(entityType))),
    args: {
      where: { type: filterType },
      first: { type: GraphQLInt2 },
      skip: { type: GraphQLInt2 },
...
// src/server/graphql/buildSingularField.ts
import {
  GraphQLID as GraphQLID3,
  GraphQLNonNull as GraphQLNonNull3
} from "graphql";
var buildSingularField = (entity, entityType) => {
  const resolver = async (_, args, context) => {
    const { store } = context;
    const { id } = args;
    if (!id)
  ...
  return {
    type: entityType,
    args: {
      id: { type: new GraphQLNonNull3(GraphQLID3) }
    },
    resolve: resolver
  };
};
...
var buildGqlSchema = (schema) => {
  ...
  const queryType = new GraphQLObjectType5({
    name: "Query",
    fields: queryFields
  });
  const gqlSchema = new GraphQLSchema3({
    query: queryType
  });
  return gqlSchema;
};

// src/reload/readGraphqlSchema.ts
import { buildSchema as buildSchema2 } from "graphql";
...
var readGraphqlSchema = ({ options }) => {
  const schemaBody = readFileSync2(options.schemaFile);
  const schemaSource = schemaHeader + schemaBody.toString();
  const schema = buildSchema2(schemaSource);
  return schema;
};

// src/reload/service.ts
var ReloadService = class extends Emittery6 {
  resources;
  latestFileHashes = {};
  kill;
    ...
    watcher.on("change", async (filePath) => {
      if (filePath === this.resources.options.configFile) {
        this.resources.logger.logMessage(
          "error",
          "detected change in ponder.config.ts. " + pico.bold("Restart the server.")
        );
...
import Emittery7 from "emittery";
import express from "express";
import { graphqlHTTP } from "express-graphql";
import { createHttpTerminator } from "http-terminator";
var ServerService = class extends Emittery7 {
  resources;
  userStore;
  app;
  terminate;
...
// src/ui/app.tsx
import { Box as Box3, Newline, render as inkRender, Text as Text4 } from "ink";
...
var HandlersBar = ({ ui }) => {
  ...
  const titleText = () => {
    if (isUpToDate)
      return /* @__PURE__ */ React2.createElement(Text2, { color: "greenBright" }, "(up to date)");
    if (isStarted)
      return /* @__PURE__ */ React2.createElement(Text2, { color: "yellowBright" }, "(up to ", ui.handlersToTimestamp === 0 ? "" : dateText, ")");
    return /* @__PURE__ */ React2.createElement(Text2, null, "(not started)");
  };
  const countText = () => {
    if (isUpToDate)
      return /* @__PURE__ */ React2.createElement(Text2, null, " ", "| ", ui.handlersCurrent, "/", ui.handlersHandledTotal, " events (", ui.handlersTotal, " total)");
    if (isStarted)
      return /* @__PURE__ */ React2.createElement(Text2, null, " ", "| ", ui.handlersCurrent, "/", "?".repeat(ui.handlersCurrent.toString().length), " events (", ui.handlersTotal, " total)");
    return null;
  };
  return /* @__PURE__ */ React2.createElement(Box, { flexDirection: "column" }, /* @__PURE__ */ React2.createElement(Box, { flexDirection: "row" }, /* @__PURE__ */ React2.createElement(Text2, { bold: true }, "Handlers "), /* @__PURE__ */ React2.createElement(Text2, null, titleText())), /* @__PURE__ */ React2.createElement(Box, { flexDirection: "row" }, /* @__PURE__ */ React2.createElement(
    ProgressBar,
    {
      current: ui.handlersCurrent,
      end: Math.max(ui.handlersHandledTotal, 1)
    }
  ), /* @__PURE__ */ React2.createElement(Text2, null, " ", completionText, countText())), /* @__PURE__ */ React2.createElement(Text2, null, " "));
};
...
var HistoricalBar = ({
  ...
  const currentCompletionRate = (cachedBlocks + completedBlocks) / (totalBlocks || 1);
  const eta = (Date.now() - startTimestamp) / // Elapsed time in milliseconds
  (completedBlocks / (totalBlocks - cachedBlocks));
  const etaText = completedBlocks > 5 && eta > 0 ? ` | ~${formatEta(eta)}` : null;
  const completionDecimal = Math.round(currentCompletionRate * 1e3) / 10;
  const completionText = Number.isInteger(completionDecimal) && completionDecimal < 100 ? `${completionDecimal}.0%` : `${completionDecimal}%`;
  return /* @__PURE__ */ React3.createElement(Box2, { flexDirection: "column" }, /* @__PURE__ */ React3.createElement(Text3, null, title), /* @__PURE__ */ React3.createElement(Box2, { flexDirection: "row" }, /* @__PURE__ */ React3.createElement(
    ProgressBar,
    {
      current: cachedBlocks + completedBlocks,
      end: totalBlocks || 1
    }
  ), /* @__PURE__ */ React3.createElement(Text3, null, " ", completionText, etaText)));
};
...
var App = (ui) => {
  ...
  if (handlerError) {
    return /* @__PURE__ */ React4.createElement(Box3, { flexDirection: "column" }, /* @__PURE__ */ React4.createElement(Text4, null, " "), /* @__PURE__ */ React4.createElement(Text4, { color: "cyan" }, "Resolve the error and save your changes to reload the server."));
  }
  return /* @__PURE__ */ React4.createElement(Box3, { flexDirection: "column" }, /* @__PURE__ */ React4.createElement(Text4, null, " "), /* @__PURE__ */ React4.createElement(Box3, { flexDirection: "row" }, /* @__PURE__ */ React4.createElement(Text4, { bold: true }, "Historical sync "), isHistoricalSyncComplete ? /* @__PURE__ */ React4.createElement(Text4, { color: "greenBright" }, "(done in ", historicalSyncDuration, ")", /* @__PURE__ */ React4.createElement(Newline, null)) : /* @__PURE__ */ React4.createElement(Text4, { color: "yellowBright" }, "(in progress)")), !isHistoricalSyncComplete && /* @__PURE__ */ React4.createElement(Box3, { flexDirection: "column" }, Object.entries(historicalSyncLogFilterStats).map(
    ([logFilterName, stat]) => /* @__PURE__ */ React4.createElement(
      HistoricalBar,
      {
        key: logFilterName,
        title: logFilterName,
  ...
  ), /* @__PURE__ */ React4.createElement(Text4, null, " ")), /* @__PURE__ */ React4.createElement(HandlersBar, { ui }), networks.length > 0 && /* @__PURE__ */ React4.createElement(Box3, { flexDirection: "column" }, /* @__PURE__ */ React4.createElement(Text4, { bold: true }, "Networks"), networks.map((network) => /* @__PURE__ */ React4.createElement(Box3, { flexDirection: "row", key: network }, /* @__PURE__ */ React4.createElement(Text4, null, network.slice(0, 1).toUpperCase() + network.slice(1), " (live)"))), /* @__PURE__ */ React4.createElement(Text4, null, " ")), handlersCurrent > 0 && /* @__PURE__ */ React4.createElement(Box3, { flexDirection: "column" }, /* @__PURE__ */ React4.createElement(Text4, { bold: true }, "GraphQL "), /* @__PURE__ */ React4.createElement(Box3, { flexDirection: "row" }, /* @__PURE__ */ React4.createElement(Text4, null, "Server live at http://localhost:", port))));
};
var setupInkApp = (ui) => {
  const { rerender, unmount: inkUnmount, clear } = inkRender(/* @__PURE__ */ React4.createElement(App, { ...ui }));
  const render = (ui2) => {
    rerender(/* @__PURE__ */ React4.createElement(App, { ...ui2 }));
  };
  const unmount = () => {
    clear();
    inkUnmount();
...
var UiService = class {
  ...
  constructor({
    ...
    this.renderInterval = setInterval(async () => {
      const totalBlocksMetric = await this.resources.metrics.ponder_historical_total_blocks.get();
      const cachedBlocksMetric = await this.resources.metrics.ponder_historical_cached_blocks.get();
      const completedBlocksMetric = await this.resources.metrics.ponder_historical_completed_blocks.get();
      Object.keys(this.ui.historicalSyncLogFilterStats).forEach((name) => {
        const totalBlocks = totalBlocksMetric.values.find((v) => v.labels.logFilter === name)?.value ?? 0;
        const cachedBlocks = cachedBlocksMetric.values.find((v) => v.labels.logFilter === name)?.value ?? 0;
        const completedBlocks = completedBlocksMetric.values.find((v) => v.labels.logFilter === name)?.value ?? 0;
        ...
        this.ui.historicalSyncLogFilterStats[name].completedBlocks = completedBlocks;
      });
      this.render();
    }, 17);
    this.etaInterval = setInterval(() => {
      if (!this.resources.options.uiEnabled)
        this.logHistoricalSyncProgress();
    }, 1e3);
  }
  kill() {
    this.unmount();
  ...
  logHistoricalSyncProgress() {
    if (this.ui.isHistoricalSyncComplete)
      return;
    this.logFilters.forEach((contract) => {
      const stat = this.ui.historicalSyncLogFilterStats[contract.name];
      const { startTimestamp, cachedBlocks, totalBlocks, completedBlocks } = stat;
      const currentCompletionRate = (cachedBlocks + completedBlocks) / totalBlocks;
      const eta = (Date.now() - startTimestamp * 1e3) / // Elapsed time in seconds
      (completedBlocks / (totalBlocks - cachedBlocks));
      const isDone = currentCompletionRate === 1;
      if (isDone)
        return;
      const etaText = stat.completedBlocks > 5 && eta > 0 ? `~${formatEta(eta)}` : "not started";
...
import Emittery8 from "emittery";
import { encodeEventTopics as encodeEventTopics2, getAbiItem } from "viem";

// src/errors/eventHandler.ts
var EventHandlerError = class extends BaseError {
  ...
  constructor({
    ...
    const shortMessage = `Error while handling \`${eventName}\` event` + (blockNumber ? ` at block ${blockNumber}` : "");
    const metaMessages = [];
    if (stackTrace)
      metaMessages.push(`Trace:
${stackTrace}`);
    if (codeFrame)
      metaMessages.push(codeFrame);
    if (params)
...
var getStackTraceAndCodeFrame = (error, options) => {
  if (!error.stack)
    return { stackTrace: void 0, codeFrame: void 0 };
  const buildDir = path7.join(options.ponderDir, "out");
  ...
  const formattedStackTrace = sourceMappedStackTrace.map(
    (frame) => `    at ${frame.methodName} (${frame.file}:${frame.lineNumber}${frame.column !== null ? `:${frame.column}` : ""})`
  ).join("\n");
...
function getSourceMappedStackFrame(file, lineNumber, columnNumber) {
  let fileContents;
  try {
    fileContents = readFileSync4(file, { encoding: "utf-8" });
  } catch (_) {
    return null;
  }
  ...
  if (!sourceFileRelative || !sourceLineNumber)
    return null;
  const sourceFile = path7.resolve(path7.dirname(file), sourceFileRelative);
  return {
    sourceFile,
    sourceLineNumber,
    sourceColumnNumber,
...
// src/user-handlers/service.ts
var EventHandlerService = class extends Emittery8 {
  resources;
  userStore;
    ...
    if (newHandlers) {
      ...
      this.logFilters.forEach((logFilter) => {
        const handledEventSignatureTopics = Object.keys(
          ...
          const topics = encodeEventTopics2({
            abi: logFilter.abi,
            eventName
          });
  ...
  processEvents = async ({ toTimestamp }) => {
    ...
    try {
      await this.eventProcessingMutex.runExclusive(async () => {
        if (!this.queue)
          return;
        const fromTimestamp = this.eventsHandledToTimestamp === 0 ? 0 : this.eventsHandledToTimestamp + 1;
  ...
  createEventQueue = ({
    ...
    const models = schema.entities.reduce(
      (acc, { name: modelName }) => {
        acc[modelName] = {
          findUnique: ({ id }) => {
...
import { CompiledQuery as CompiledQuery2, Kysely as Kysely4, PostgresDialect as PostgresDialect2, sql as sql4 } from "kysely";
...
var PostgresUserStore = class {
    ...
    this.db = new Kysely4({
      dialect: new PostgresDialect2({
        ...
        onCreateConnection: databaseSchema ? async (connection) => {
          ...
          await connection.executeQuery(
            CompiledQuery2.raw(`SET search_path = ${databaseSchema}`)
          );
        } : void 0
      })
    });
                ...
                tableBuilder = tableBuilder.addColumn(
                  ...
                  (col) => {
                    if (field.notNull)
                      col = col.notNull();
                    col = col.check(
                      sql4`${sql4.ref(field.name)} in (${sql4.join(
                        field.enumValues.map((v) => sql4.lit(v))
                      )})`
                    );
                    return col;
                  }
  ...
  findMany = async ({
    ...
    if (orderBy) {
      query2 = query2.orderBy(
        orderBy,
        orderDirection === "asc" || orderDirection === void 0 ? sql4`asc nulls first` : sql4`desc nulls last`
      );
    }
  ...
  deserializeInstance = ({
    ...
    entity.fields.forEach((field) => {
      ...
      if (field.kind === 0 /* SCALAR */ && field.scalarTypeName === "Boolean") {
        deserializedInstance[field.name] = value === 1 ? true : false;
        return;
      }
      if (field.kind === 0 /* SCALAR */ && field.scalarTypeName === "BigInt") {
        deserializedInstance[field.name] = blobToBigInt(
          value
        );
        return;
      }
      if (field.kind === 2 /* LIST */) {
        deserializedInstance[field.name] = JSON.parse(value);
        return;
      }
...
import { Kysely as Kysely5, sql as sql5, SqliteDialect as SqliteDialect2 } from "kysely";
...
var SqliteUserStore = class {
  ...
  constructor({ db }) {
    this.db = new Kysely5({
      dialect: new SqliteDialect2({ database: db })
    });
  }
  /**
   * Resets the database by dropping existing tables and creating new tables.
  ...
  reload = async ({ schema } = {}) => {
    ...
    await this.db.transaction().execute(async (tx) => {
      ...
      if (schema)
        this.schema = schema;
      this.versionId = randomBytes2(4).toString("hex");
      await Promise.all(
        this.schema.entities.map(async (model) => {
          const tableName = `${model.name}_${this.versionId}`;
          let tableBuilder = tx.schema.createTable(tableName);
          model.fields.forEach((field) => {
            switch (field.kind) {
                ...
                tableBuilder = tableBuilder.addColumn(
                  ...
                  (col) => {
                    if (field.notNull)
                      col = col.notNull();
                    col = col.check(
                      sql5`${sql5.ref(field.name)} in (${sql5.join(
                        field.enumValues.map((v) => sql5.lit(v))
                      )})`
                    );
                    return col;
                  }
              ...
              case 3 /* RELATIONSHIP */: {
                tableBuilder = tableBuilder.addColumn(
                  field.name,
                  gqlScalarToSqlType2[field.relatedEntityIdTypeName],
                  (col) => {
                    if (field.notNull)
                      col = col.notNull();
                    return col;
...
var LoggerService = class {
  ...
  logMessage(kind, message) {
    ...
    switch (kind) {
      case "event": {
        this.info(pico2.magenta(padded) + " - " + message);
        break;
      }
      case "error": {
        this.error(pico2.red(padded) + " - " + message);
        break;
      }
      case "warning": {
        this.error(pico2.yellow(padded) + " - " + message);
        break;
      }
      case "historical": {
        this.info(pico2.yellow(padded) + " - " + message);
        break;
      }
      case "realtime": {
        this.info(pico2.cyan(padded) + " - " + message);
        break;
      }
      case "indexer": {
        this.info(pico2.blue(padded) + " - " + message);
        break;
      }
    }
  }
...
var Ponder = class {
  ...
  registerServiceDependencies() {
    ...
    this.networkSyncServices.forEach((networkSyncService) => {
      const { network, historicalSyncService, realtimeSyncService } = networkSyncService;
  ...
  registerUiHandlers() {
    ...
    this.networkSyncServices.forEach((networkSyncService) => {
      const { historicalSyncService, realtimeSyncService, logFilters } = networkSyncService;
      ...
      historicalSyncService.on("syncStarted", () => {
        logFilters.forEach(({ name }) => {
          this.uiService.ui.historicalSyncLogFilterStats[name].startTimestamp = Date.now();
          this.resources.logger.logMessage(
            "historical",
            `started historical sync for ${pico3.bold(name)} (${formatPercentage(
    ...
    setInterval(() => {
      this.networkSyncServices.forEach((networkSyncService) => {
        ...
        if (realtimeSyncService.stats.isConnected && !this.uiService.ui.networks.includes(network.name)) {
      ...
      this.uiService.ui.handlersHandledTotal = this.eventHandlerService.metrics.eventsAddedToQueue;
      this.uiService.ui.handlersCurrent = this.eventHandlerService.metrics.eventsProcessedFromQueue;
      this.uiService.ui.handlersTotal = this.eventHandlerService.metrics.totalMatchedEvents;
      this.uiService.ui.handlersToTimestamp = this.eventHandlerService.metrics.latestHandledEventTimestamp;
    }, 17);
...
//# sourceMappingURL=chunk-QVKH5AW3.mjs.map